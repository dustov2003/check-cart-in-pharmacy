...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(  # my test
            68,
            {
                "category_123": "WRONG_CATEGORY",
            },
            id="wrong-category",
        ),
        pytest.param(
        ...
        pytest.param(  # my test
            62,
            {
                "special_2": "ITEM_SPECIAL_WRONG_SPECIFIC",
            },
            id="doctor-speciality-mismatch",
        ),
        pytest.param(
...
def test(user_id, items):
    got = requests.get(ENDPOINT, timeout=1, params={"user_id": user_id, "item_id": list(items.keys())})
    ...
    for resp_item in got.json():
        ...
        assert not has_uppercase_letters(resp_item_id), "item_id in response has uppercase letters"
        assert problem_exists(resp_problem), (
            f"Unexpected problem code. " f"Got: {resp_problem} but [{PROBLEMS}] expected"
        )
        assert resp_item_id in items, f'Got item_id: "{resp_item_id}" that was not in the request'
    ...
    assert not missing_item_ids, f"Some item_ids is missing in response: [{missing_item_ids}]"
    ...
    assert not extra_item_ids, f"Got unexpected item_ids in response: [{extra_item_ids}]"
