...
APPLICATION_NAME = pharmacy_cart_checker
...
db:  ##@Database Create database with docker-compose
	...
	sudo chmod 777 -R database/postgresql-docker-image/database_data
...
open_db:  ##@Database Open database inside docker-image
	docker exec -it $(DATABASE_CONTAINER) psql -d $(DATABASE_NAME) -U $(DATABASE_USERNAME)
